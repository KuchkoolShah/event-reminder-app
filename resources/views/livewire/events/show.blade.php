<div class="py-12">
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white rounded-lg shadow-sm p-6 md:p-8">
            ...
            <!-- Badges: visibility & reminder status -->
            <div class="mb-4 flex flex-wrap gap-2">
                <span
                    class="px-2 py-1 text-xs font-medium rounded-md
                    {{ $event->is_public ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700' }}">
                    {{ $event->is_public ? 'Public' : 'Private' }}
                </span>
                <span
                    class="px-2 py-1 text-xs font-medium rounded-md
                    {{ $event->reminder_sent ? 'bg-blue-100 text-blue-700' : 'bg-yellow-100 text-yellow-700' }}">
            ...
            <div class="mt-8 flex flex-wrap gap-3">
                ...
                @endcan
                <!-- Edit button – only show if user can edit (global permission or ownership) -->
                @can('events-edit')
                ...
                <!-- Delete button – only show if user can delete (policy or permission) -->
                @can('delete', $event)
